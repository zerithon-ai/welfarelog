...
import { User, WorkType, WorkShift } from '@/types';
          ...
          <div className="grid grid-cols-7 gap-1">
            {days.map((day, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`aspect-square flex flex-col items-center justify-center text-sm border border-gray-100 rounded-lg relative ${
                    day && (showWorkTypeBar || isQuickInputMode) ? 'cursor-pointer hover:bg-gray-50' : ''
                  } ${isToday ? 'bg-blue-50 border-blue-200' : ''} ${
                    isSelected ? 'bg-green-50 border-green-300 border-2' : ''
                  }`}
        ...
        {showWorkTypeModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-sm">
              <h3 className="text-lg font-bold text-gray-900 mb-4">근무 선택</h3>
              {workTypes.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-gray-600 mb-4">등록된 근무 타입이 없습니다</p>
                  <Button
                    variant="primary"
                    onClick={() => {
                      setShowWorkTypeModal(false);
                      router.push('/settings/work-types');
                    }}
                  >
                    근무 타입 추가하기
                  </Button>
                  ...
                  {workTypes.map((workType) => (
                    <div key={workType.id} className="space-y-2">
                      <button
                        ...
                        className="w-full p-3 text-left bg-gray-50 hover:bg-gray-100 rounded-lg flex items-center space-x-3"
                      >
                        <div
                          className="w-4 h-4 rounded-full"
